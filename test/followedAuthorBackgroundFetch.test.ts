...
(async () => {
  ...
  class MockPool {
    async list(_relays, filters) {
      if (
        filters.some(
          (f) =>
            (f.authors || []).includes('a1') &&
            ((f.kinds || []).includes(10003) || (f.kinds || []).includes(30004)),
        )
      ) {
        return [
          {
  ...
  const sandbox = {
    require: (p) => {
      ...
      if (p.includes('useSettings'))
        return {
          useSettings: (sel) => sel({ textSize: 16, density: 'comfortable', hydrate: () => {} }),
        };
      if (p.includes('nostr/auth'))
        return { getPrivKey: () => 'priv', setPrivKey: () => {} };
  ...
  await TestRenderer.act(async () => {
    await ctx.saveContacts(['a1']);
    await Promise.resolve();
  });

  assert.deepStrictEqual(ctx.followBooks['b1'], ['a1']);
  assert.deepStrictEqual(ctx.followBooks['b2'], ['a1']);
